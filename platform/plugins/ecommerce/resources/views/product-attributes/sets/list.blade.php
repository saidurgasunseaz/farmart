@if (!$isNotDefaultLanguage)
    <x-core::custom-template id="product_attribute_template">
        <x-core::table.body.row data-id="__id__">
            <x-core::table.body.cell width="5%">
                <x-core::form.radio name="related_attribute_is_default" value="__position__" __checked__ :single="true"
                    :checked="false" />
    ...
    <textarea class="d-none" id="deleted_attributes" name="deleted_attributes"></textarea>
@endif

<textarea class="d-none" id="attributes" name="attributes">{!! json_encode($attributes) !!}</textarea>
    ...
    <x-core::table.body class="swatches-list">
        @foreach ($attributes as $attribute)
            <x-core::table.body.row data-id="{{ $attribute->id }}">
                @if (!$isNotDefaultLanguage)
                    <x-core::table.body.cell>
                        <x-core::form.radio name="related_attribute_is_default" :value="$attribute->order" :checked="$attribute->is_default"
                            :single="true" />
                    </x-core::table.body.cell>
                @endif
                ...
                @if (!$isNotDefaultLanguage)
                    <x-core::table.body.cell>
                        <input data-bb-color-picker style="display: none" name="swatch-value" type="text"
