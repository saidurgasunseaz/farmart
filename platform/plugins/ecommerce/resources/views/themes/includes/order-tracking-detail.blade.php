...
<div class="col-lg-12 wow fadeInRight">
    <div class="dashboard_content">
        <h3 class="dashboard_title">Order invoice
            <!-- <a class="back_btn common_btn"
                href="dashboard_order.html">Go Back</a> -->
        </h3>
            ...
            <div class="dashboard_order_invoice">
                <div class="dashboard_invoice_logo_area">
                    <div class="invoice_logo">
                        ...
                        <!-- <img src="assets/images/tetra.png" alt="logo" class="img-fluid w-100"> -->
                    </div>
                    <div class="text">
                        <h2>invoice</h2>
                        <p>invoice no: {{ $order->code }}</p>
                        <p>date: {{ $order->created_at->translatedFormat('d M Y H:i:s') }}</p>
                    </div>
                </div>
                <div class="dashboard_invoice_header">
                    <div class="text">
                        <h2>Bill To</h2>
                        <ul>
                            <li><span>Order number:</span> {{ $order->code }}</li>
                            <li><span>Time: </span> {{ $order->created_at->translatedFormat('d M Y H:i:s') }}</li>
                            <li><span>Order&nbsp;status: </span> {{ $order->status->label() }}</li>
                            @if($order->cancellation_reason)
                            <li><span>Cancellation&nbsp;Reason: </span> {{ $order->cancellation_reason_message }} </li>
                            @endif
                            @if (is_plugin_active('payment') && $order->payment->id)
                            <li><span> Payment method </span> {{ $order->payment->payment_channel->label() }} </li>
                            <li><span> Payment status </span> {{ $order->payment->status->label() }}</li>
                            @endif
                    ...
                    <div class="text">
                        ...
                        <ul>
                            ...
                            <li><span>Phone:</span> {{ $order->address->phone }}</li>
                            <li><span>Address:</span> {{ $order->address->full_address }} </li>
                        </ul>
                    </div>
                </div>
                                ...
                                <tr>
                                    <td>
                                        <img src="{{ RvMedia::getImageUrl($orderProduct->product_image, 'thumb', false, RvMedia::getDefaultImage()) }}"
                                            alt="{{ $orderProduct->product_name }}" width="40px" style="width:50px !important;">
                                    </td>
                                    ...
                                    <td>{{ $orderProduct->amount_format }}</td>
                                    <td>{{ $orderProduct->qty }}</td>
                                    <td>{{ $orderProduct->total_format }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                ...
                <div>
                    <div class="bb-order-totals">
                        ...
                        <div class="bb-order-total-item">
                            ...
                            <span class="value" style="float:right;">{{ format_price($order->tax_amount) }}</span>
                        </div>
                        @endif
                        ...
                        <div class="bb-order-total-item">
                            ...
                            <span class="value" style="float:right;">{{ format_price($order->shipping_amount) }}</span>
                        </div>
                        @endif
                        <div class="bb-order-total-item grand-total">
                            <span class="label">{{ __('Total Amount') }}:</span>
                            <span class="value" style="float:right;">{{ format_price($order->amount) }}</span>
                        </div>
                    </div>
                </div>
                <div class="dashboard_invoice_footer">
                    ...
                    <a class="common_btn" href="{{ route('customer.print-order', $order->id) }}?type=print" target="_blank">Print PDF</a>
                    @endif
                </div>
            </div>
        </div>
